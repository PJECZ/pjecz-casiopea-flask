...
def copiar_cit_clientes_registros(conn_old, cursor_old, conn_new, cursor_new):
    ...
    while True:
        ...
        if offset > 0:
            click.echo()
        click.echo(click.style("Copiando registros en cit_clientes_registros: ", fg="white"), nl=False)
        insert_query = """
        ...
        try:
            for row in rows:
                ...
                cursor_new.execute("SELECT id FROM cit_clientes_registros WHERE id_original = %s", (row[0],))
